import { Directive, EmbeddedViewRef, NgIterable, Output, TrackByFunction } from '@angular/core';
 ...
 */
@Directive()
export abstract class RxVirtualScrollStrategy<T, U extends NgIterable<T> = NgIterable<T>> {
...
/** @internal */
@Directive()
export abstract class RxVirtualViewRepeater<T, U extends NgIterable<T> = NgIterable<T>> {
  values$: Observable<U>;
  rendered$: Observable<any>;
...
> extends RxDefaultListViewContext<T, U, K> {
  constructor(item: T, public rxVirtualForOf: U, customProps?: { count: number; index: number }) {
