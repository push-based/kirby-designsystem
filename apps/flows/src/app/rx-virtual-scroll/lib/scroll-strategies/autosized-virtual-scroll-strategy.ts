...
export class AutosizeVirtualScrollStrategy<T, U extends NgIterable<T> = NgIterable<T>>
  ...
  implements OnDestroy
{
      ...
      if (changes) {
        ...
        changes.forEachOperation((item, adjustedPreviousIndex, currentIndex) => {
          if (item.previousIndex == null) {
            ...
            const removeIdx =
              adjustedPreviousIndex == null ? this._virtualItems.length - 1 : adjustedPreviousIndex;
            removeFromArray(this._virtualItems, removeIdx);
          } else if (adjustedPreviousIndex !== null) {
            this._virtualItems[currentIndex] = this._virtualItems[adjustedPreviousIndex];
          }
        });
      }
    });
  ...
  private calcRenderedRange(): void {
    const dataLengthChanged$ = this.viewRepeater.values$.pipe(
      map(
        (values) =>
          (Array.isArray(values) ? values : values != null ? Array.from(values) : []).length
      ),
      distinctUntilChanged()
    );
    ...
    combineLatest([
      ...
      .pipe(
        map(([length, containerHeight]) => {
          ...
          this.lastScreenItem = this.calculateAnchoredItem(this.anchorItem, containerHeight);
          if (delta < 0) {
              ...
              switchMap((views) =>
                merge(
                  ...
                  return o$.pipe(
                    ...
                    tap(() => {
                      const sortedIds = Array.from(_resized.keys()).sort((a, b) => a - b);
                      let i = sortedIds[0];
                      ...
                      for (i; i < views.length; i++) {
                        ...
                        if (position < this.scrollTop && index >= this.scrolledIndex) {
                          this.scrolledIndex = index + 1;
                        } else if (index < this.scrolledIndex && position > this.scrollTop) {
  ...
  private calculateAnchoredItem(initialAnchor, delta): AnchorItem {
    ...
    if (delta < 0) {
      ...
      while (delta > 0 && i < this.contentLength && items[i]?.size && items[i].size < delta) {
        delta -= items[i].size;
        i++;
      }
  ...
  private adjustElementPosition({
    ...
    if (!size) {
      const isCached =
        this._cachedRange && index >= this._cachedRange.start && index <= this._cachedRange.end;
  ...
  private observeViewSize$(
    ...
    return observeElementSize(this.getElement(view), {
      ...
      filter(
        ({ target, size }) =>
          target.isConnected && Math.ceil(size) !== this._virtualItems[adjustedIndex].size
      ),
      map(({ size }) => ({
        size: Math.ceil(size),
        view,
  ...
  private getDiffer(values: unknown[]): IterableDiffer<unknown> | null {
    ...
    return values
      ? (this.dataDiffer = this.differs.find(values).create(this.viewRepeater._trackBy))
      : null;
  }
  /** @internal */
  private animationFrameTick() {
    return this.ngZone.runOutsideAngular(() => scheduled([], animationFrameScheduler));
  }
}
