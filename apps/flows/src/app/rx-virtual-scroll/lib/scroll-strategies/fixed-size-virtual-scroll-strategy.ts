...
import { Directive, EmbeddedViewRef, Input, NgIterable, OnDestroy } from '@angular/core';
import {
  combineLatest,
...
import { RxVirtualScrollStrategy } from '../model-local';
...
export class FixedSizeVirtualScrollStrategy<T, U extends NgIterable<T> = NgIterable<T>>
        ...
        map(([length, containerSize]) => {
          ...
          if (this.direction === 'up') {
            range.start = Math.floor(
              Math.max(0, this.scrollTop - this.runwayItems * this.itemSize) / this.itemSize
            );
            range.end = Math.min(
              length,
              Math.ceil(
                (this.scrollTop + containerSize + this.runwayItemsOpposite * this.itemSize) /
                  this.itemSize
              )
            );
          } else {
            range.start = Math.floor(
              Math.max(0, this.scrollTop - this.runwayItemsOpposite * this.itemSize) / this.itemSize
            );
            ...
            range.end = Math.min(
              length,
              Math.ceil(
                (this.scrollTop + containerSize + this.runwayItems * this.itemSize) / this.itemSize
              )
            );
          }
