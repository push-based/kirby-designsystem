  ...
  implements RxVirtualViewRepeater<T>, OnInit, OnDestroy
{
  ...
  @Input()
  set rxVirtualFor(
    potentialObservable: Observable<NgIterable<T>> | NgIterable<T> | null | undefined
  ) {
   ...
   * @param renderParent
   */
  @Input('rxVirtualForParent') renderParent = this.strategyProvider.config.parent;
  ...
  set trackBy(trackByFnOrKey: string | ((idx: number, i: T) => any)) {
    this._trackBy =
      typeof trackByFnOrKey !== 'function' ? (i, a) => a[trackByFnOrKey] : trackByFnOrKey;
  }
  ...
  /** @internal */
  static ngTemplateContextGuard<T, U extends NgIterable<T> = NgIterable<T>, K = keyof T>(
  ...
  /** @internal */
  ngOnInit() {
    this.listManager = createVirtualListManager<T, RxVirtualForViewContext<T, U>>({
    ...
    this.listManager.viewsRendered$.pipe(takeUntil(this._destroy$)).subscribe(this.viewsRendered$);
    this.listManager.viewRendered$.pipe(takeUntil(this._destroy$)).subscribe(this.viewRendered$);
