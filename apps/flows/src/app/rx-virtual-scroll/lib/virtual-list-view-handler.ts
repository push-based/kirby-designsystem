...
import { onStrategy, RxStrategyCredentials } from '@rx-angular/cdk/render-strategies';
...
import { ListRange } from './model-local';
...
export function getVirtualTemplateHandler<C extends RxListViewContext<T>, T>(
          ...
          ? changes.map((change) => {
              return onStrategy(
                ...
                (_change) => {
                  ...
                  const update = () => ({
                    view: viewContainerRef.get(index || 0) as EmbeddedViewRef<C>,
                    index,
                    change: type,
                    item,
                  });
                  switch (type) {
                    case RxListTemplateChangeType.insert:
                      ...
                      _insertView(item, index, count, renderedRange.start + index);
                    ...
                    case RxListTemplateChangeType.update:
                      ...
                      _updateView(item, index, count, renderedRange.start + index);
                      _viewRendered$.next(update());
                      break;
  ...
  function _updateUnchangedContext(index: number, count: number, contextIndex: number) {
  ...
  function _updateView(item: T, index: number, count: number, contextIndex: number): void {
    const view = <EmbeddedViewRef<C>>viewContainerRef.get(index);
  ...
  function _insertView(
    ...
    const view = viewContainerRef.createEmbeddedView(initialTemplateRef, context, currentIndex);
    view.detectChanges();
    return view;
  }
 ...
 * @param items
 */
function getListChanges<T>(changes: IterableChanges<T>, items: T[]): RxListTemplateChanges<T> {
  ...
  function getMoveChange(
    ...
    return [RxListTemplateChangeType.move, [item, currentIndex, adjustedPreviousIndex]];
  }

  function getUpdateChange(item: T, currentIndex: number): RxListTemplateChange<T> {
  ...
  function getInsertChange(item: T, currentIndex: number | null): RxListTemplateChange<T> {
    return [
  ...
  function getRemoveChange(item: T, adjustedPreviousIndex: number): RxListTemplateChange<T> {
    return [
