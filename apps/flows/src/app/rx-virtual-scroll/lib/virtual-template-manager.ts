...
import { RxStrategyCredentials, strategyHandling } from '@rx-angular/cdk/render-strategies';
import { ListRange } from './model-local';
import { createErrorHandler } from './render-error';
import { RxListTemplateChangeType, RxRenderSettings, RxTemplateSettings } from './model';
...
export type VirtualListManager<T, C> = RxListManager<T> & {
  render(data$: Observable<NgIterable<T>>, range$: Observable<ListRange>): Observable<any>;
...
export function createVirtualListManager<T, C extends RxListViewContext<T>>(config: {
  ...
  const { defaultStrategyName, strategies, cdRef: injectingViewCdRef, parent } = renderSettings;
  ...
  const viewRenderedSub = listViewHandler.viewRendered$
    .pipe(
      filter(({ view, change }) => !!view && change !== RxListTemplateChangeType.remove),
      map(({ view, item, index }) => ({
        view,
        item,
        index,
  ...
  return {
    ...
    render(values$: Observable<NgIterable<T>>, range$: Observable<ListRange>): Observable<any> {
      return combineLatest([
        values$.pipe(
          map((values) =>
            Array.isArray(values) ? values : values != null ? Array.from(values) : []
          )
        ),
        range$,
  ...
  function render(): OperatorFunction<[T[], ListRange, RxStrategyCredentials], any> {
    return (o$: Observable<[T[], ListRange, RxStrategyCredentials]>): Observable<any> =>
      o$.pipe(
        ...
        switchMap(({ changes, count, itemsToRender, strategy }) => {
          ...
          return combineLatest(
            ...
            tap(() => {
              ...
              let i = 0;
              for (i; i < end; i++) {
                viewsRendered.push(viewContainerRef.get(i) as EmbeddedViewRef<C>);
              }
              _viewsRendered$.next(viewsRendered);
            }),
            notifyAllParentsIfNeeded(injectingViewCdRef, strategy, () => notifyParent),
            handleError(),
            map(() => itemsToRender)
          );
        })
